...
from FactoryVerse.factory.dsl import playing_factorio, inventory, reachable
...
async def place_furnace_simple():
    ...
    with playing_factorio():
        ...
        try:
            ...
            print(
                f"Placed {furnace.name} at ({furnace.position.x}, {furnace.position.y})"
            )
...
async def place_drill_with_validation():
    ...
    with playing_factorio():
        ...
        if cues.reachable_count > 0:
            ...
            drill = drill_item.place(position=pos, direction=Direction.NORTH)
...
async def place_ghosts_for_later():
    ...
    with playing_factorio():
        ...
        for pos in cues.positions[:5]:
            try:
                ghost = drill_item.place_ghost(
                    position=pos, direction=Direction.NORTH, label="coal-mining-setup"
                )
                ghosts_placed += 1
        ...
        # from FactoryVerse.factory.dsl import ghosts
