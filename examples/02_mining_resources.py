...
from FactoryVerse.factory.dsl import playing_factorio, reachable, inventory
...
async def mine_iron_ore_batch():
    ...
    with playing_factorio():
        ...
        if hasattr(resource, "total"):
            print(
                f"  Patch with {resource.total} total ore across {resource.count} tiles"
            )
...
async def mine_trees_for_wood():
    ...
    with playing_factorio():
        ...
        tree = trees[0]
        print(
            f"Found tree: {tree.name} at ({tree.position.x:.1f}, {tree.position.y:.1f})"
        )
