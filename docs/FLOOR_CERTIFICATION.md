...
| L1.9 | Standalone DB loader can create schema and ingest a snapshot dir | `load_all` / `create_schema` from a fresh process | Loads without error | offline-ish | ❌ 2026-06-10 — BinderException: FK `belt_line.start_entity_name` VARCHAR vs `map_entity.entity_name` ENUM(placeable_entity) (`db/schema.py:423/430`); fails before any data. UNKNOWN whether Tier4 sessions hit this or take a different path — determines severity. |
