...
| L0.3 | State survives save/load | Census → save → reload → census; diff | Identical census; all relational reads (L2) still resolve (no unit_number leakage) | live | 🔧 2026-06-11 — NO-HARNESS, infra gaps found: `game.server_save` fails out-of-box (generated compose `entrypoint: []` skips the init that creates `/factorio/saves` — exact fs error captured); works after mkdir but save is container-ephemeral (no saves volume) and `fv server start` has no load-save affordance (`--start-server-load-scenario` hardcoded). Needs: saves volume + init mkdir + `fv server start --save`. (`.fv-output/certification/2026-06-11/L0.3/`) |
...
| L1.6 | Parity survives churn | Scripted 500-op loop (place/remove/rotate, mixed entity types), then L1.1 | Empty diff | live | ❌ 2026-06-11 @ 8ea7a5c — UDP itself perfect at 60 ops/s (0 drops, 0 rebuilds) but **agent `pickup_entity` removals are invisible to the snapshot pipeline** (SNAP-4): `character.mine_entity()` on a non-player character raises no event; no custom event exists; fv_snapshot removal handlers only cover player/robot-mined, died, script_raised_destroy → 20 permanent phantom rows per 500-op run, rebuild cannot recover (files lack the removes). Script raise_destroy path syncs exact. Minimal repro in evidence. (`.fv-output/certification/2026-06-11/L1.6/`) |
