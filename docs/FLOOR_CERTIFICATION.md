...
> **Stack-B finding (2026-06-10, code-trace at 95% confidence, empirical confirmation in flight via L1.sync runner):** the entire `db/` subtree (create_schema, load_all, derived_loader, analytics_loader, status_watcher) has no runtime callers. Consequences: derived/aggregate tables (`belt_line`, `belt_line_segment`, `resource_patch`, `water_patch`, pole aggregates) **never exist in a session DB** — agents can't query them because they're not created, not because they're stale. Analytics tables exist in Stack A's schema but are never populated (verification reads JSONL directly). The derivation algorithms (DBSCAN patches, belt-graph DFS) are valuable and should eventually be ported INTO Stack A as the aggregate-perception layer (build-taxonomy KNOT-3) — **decision needed: quarantine `db/` as reference material vs delete**. L1.5's derived-table half collapses into this; its component-table half is still live and being tested.
