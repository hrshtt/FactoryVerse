...
| L1.3 | Rotation/config-change syncs | same harness | Direction/recipe in DB match game | live | ❌ 2026-06-10 — rotation ✅ (direction 4→8 synced); config-change ❌: **dual sequence counters** — `udp_payloads.lua:374` uses `storage.udp_sequence` while created/destroyed/rotated use `storage.snapshot_sequence`, so every config op is a sequence gap → dropped + rebuild storm; AND config ops are never written to entities-updates.jsonl, so rebuild can't recover them — recipe stays stale until full chunk re-snapshot. Two-part mod fix needed. |
