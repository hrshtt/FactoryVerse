...
| L4.4 | Geometry handoff: positions the API returns are accepted by the API | same harness: drill `drop_position` round-trip + `get_fluid_connections` boiler→steam-engine with live controls | Accepted or explicitly marked non-placeable; connection candidates non-empty and placeable | live | ✅ 2026-06-10 (server_0) — was ❌(half) same day. drop_position round-trips honestly (KNOT-2 never manifested). **PLACE-1 FIXED**: solver rewritten geometry-aware (target's own fluid-connection offsets probed live from the engine per direction; candidates must land on the source's partner cell and point back; flow-compat + dedupe + can_place; legacy fallback retained). Re-run: boiler→steam-engine count=1, candidate **placed successfully**, engine confirms `fluidbox_connections=1`; boiler→pipe control = 3 precise candidates. The March eval's single biggest failure cause is closed. |
