...
| ID | Claim | How to check | Pass criterion | Req | Status |
|----|-------|--------------|----------------|-----|--------|
| L1.1 | **Census parity** — every placed entity in game ↔ exactly one `map_entity` row | `fv dev census` + `scripts/certification/check_L1_1.py` | Empty diff both directions | live | ⏳ harness ready 2026-06-11, parity run pending — dumper mechanics validated (90,159 rows == independent direct count at full-map scale; vacuity guard live-fired; audit in census-harness/). Acceptance needs quiesced world + per-cell `--force`/`--bounds` scoping (lab-grid cells are per-cell forces). |
