...
| L1.10 | **Lab-grid session pipeline coherent (SNAP-1)**: tiles + entities + tick freshness in a real session DB | Stand up a lab-grid session (or run during one): `water_tile`/`resource_tile`/`map_entity` row counts vs surface ground truth; snapshot tick advances | Non-empty where surface is non-empty; tick within N sec of game tick | live | 🔧 — **TOP PRIORITY** (run-killer of the 2026-06-10 eval: water_tile 0 rows vs 216 real tiles, map_entity always empty, tick frozen at 660). The L1 battery covered test-ground entity ops only; tile snapshotting + full session pipeline never certified. |
