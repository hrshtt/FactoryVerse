...
| L0.3 | State survives save/load | Census → `game.server_save` → `fv server start --save <name>` → census; diff | Identical census; all relational reads (L2) still resolve (no unit_number leakage) | live | ⏳ affordances built 2026-06-11 (were missing: compose `entrypoint: []` skips the init that creates `/factorio/saves`; no saves volume; no CLI load path). Now: per-server saves volume `.fv-output/server_N/saves` ↔ `/factorio/saves` (mkdir'd host-side) + `fv server start --save <name>`. Check itself still UNRUN — needs a save→reload→census-diff execution. Diagnosis evidence: `.fv-output/certification/2026-06-11/L0.3/` |
...
| L2.2 | Fluid topology readable | offshore-pump → pipe → boiler → steam-engine rig, flow-proven, 4-layer compare | Fluid connections enumerable from Python; boiler/engine report connected | live | ❌ 2026-06-11 — live rig, flow-proven (engine 41.67 J/tick, gears produced): L1 raw-Lua truth complete; **inspect drops boiler/steam-engine fluid entirely** (dispatcher compares entity_type to NAMES, inspection.lua:1038 — runtime type is "generator"; no fluidbox read in inspect_energy_producer); connectivity absent at L2/L3/L4; FluidState has no connections field, capacity never sent (=0); DB pipe_neighbours stale (loader replays build-tick updates over fresher re_snapshot init). Pump/pipe CONTENTS do flow L1→L3 — field report "fluidboxes=[] always" was too broad. (`.fv-output/certification/2026-06-11/L2.2/`) |
| L2.3 | Power topology readable | 2 poles + machine powered by the L2.2 rig, power-proven, 4-layer compare | Pole graph (copper links + supply area) readable; machine reports powered. NOTE: pole net id ≠ machine net id even at engine level — "consistent network" must NOT be defined as id equality | live | ❌ 2026-06-11 — live rig, power-proven (asm WORKING, 1377 J, crafting): **poles inspect as bare base payloads** (same name-vs-type dispatch bug, inspection.lua:1045 — runtime type is "electric-pole") → ElectricPoleState stub defaults exactly as field-reported; even if dispatched, inspect_electric_pole never reads copper neighbours (`e.neighbours` RAISES on poles in 2.0 — needs get_wire_connector) or supply area; transform hardcodes supply_area_entities=0; machine inspectors never send electric_network_id. Machine powered-state DOES read correctly L1→L3. Pole net id survives only in DB raw_data. (`.fv-output/certification/2026-06-11/L2.3/`) |
