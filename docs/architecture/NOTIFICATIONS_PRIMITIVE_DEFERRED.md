...
**Status (corrected 2026-09-03): PARTIALLY EXECUTED.** `stream.lua`, `vocabulary.lua`, the `turn` stream and the Python `(epoch, seq)` mirror landed in Phase 2B (`28f5b2d`); gate 5 landed in `e39e942`; gates 1, 5, 6, 7 and 8 are green, including live. Still unscheduled: the `action` stream's migration, the `entities` and `files` streams, gates 2–4, `sync.py`'s accept-anything-first attach, and the census / `fv_core` workstream (which is a separate workstream and should get its own deferred note). The status line this replaces said "designed, not executed… nothing in it has been run"; that was false from 2026-08-29. The finding and the inherited scope below are unchanged. §*The design* was added after a full read of every send site in both emitting mods, the Python receive side, and the world lifecycle; it names running values and the gates that stand before any of it is believed.
