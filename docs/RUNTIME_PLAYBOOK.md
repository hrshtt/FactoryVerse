...
| `lab_grid` | lab-grid scenario only | `get_config`, `allocate_cell`, `create_agent_in_cell`, `get_cell_status`, `unassign_agent` — ⚠️ NO `release_cell` (use `unassign_agent <id>`); scenario fns take POSITIONAL args only (no ParamSpec normalization — a named-table arg is a silent no-op) |
