...
class FactorioHeadlessClusterManager:
    ...
    def _get_volumes(self, instance_index: int, for_server: bool = True) -> list:
        vols = {
            self.config.local_mods_dir: {"bind": "/factorio/mods", "mode": "rw"},
            str(self.config.fv_saves_dir / str(instance_index)): {
                "bind": "/factorio/saves",
                "mode": "rw",
            },
            str(self.config.fv_scenario_dir.resolve()): {
        ...
        if for_server:
            vols[str(self.config.fv_server_config_dir.resolve())] = {
    ...
    def check_save_exists(self, instance_index: int):
        save_dir = self.config.fv_saves_dir / str(instance_index)
    ...
    async def get_scenario2map_ctr(self, instance_index: int):
        ...
        return await self.docker.containers.run(
            config=config, name=f"conv_{self.config.image_name_prefix}{instance_index}"
        )
    ...
    async def get_server_ctr(self, instance_index: int):
        ...
        return await self.docker.containers.create_or_replace(
            config=config, name=f"{self.config.image_name_prefix}{instance_index}"
        )
    ...
    async def start(self):
        ...
        for i in range(self.num):
            (self.config.fv_saves_dir / str(i)).mkdir(parents=True, exist_ok=True)

        if self.dry_run:
            for i in range(self.num):
                name = f"{self.config.image_name_prefix}{i}"
                print(f"\nContainer name: {name}")
    ...
    async def stop(self):
        ...
        ctrs = await self.docker.containers.list(filters={"name": [f"/{self.config.image_name_prefix}"]})
    ...
    async def hot_reload_scenario(self):
        ...
        containers = await self.docker.containers.list(filters={"name": [f"/{self.config.image_name_prefix}"]})
        
        async def sync_container(ctr):
            cmd = (
                f"docker exec -u root {ctr.id} sh -c "
                f"'cp -a /factorio/scenarios/{self.config.scenario_name}/. {self.config.server_currently_playing_dir} && "
                f"chown -R factorio:factorio {self.config.server_currently_playing_dir}'"
            )
...
def parse_args():
    ...
    p.add_argument(
        ...
        default=Scenario.FACTORY_VERSE.value,
...
async def main():
    ...
    if args.mode == Mode.SCENARIO.value:
        mode = Mode.SCENARIO.value
    elif args.mode == Mode.SAVE_BASED.value:
        mode = Mode.SAVE_BASED.value
    config = DockerConfig(mode=mode)
    ...
    if args.s == Scenario.FACTORY_VERSE.value:
        config.scenario_name = Scenario.FACTORY_VERSE.value
