...
class GameDataSyncService:
    ...
    def __init__(
        ...
        self._sequence_gaps: List[
            Tuple[str, int, int]
        ] = []  # (event_type, expected, received)
    ...
    async def start(self) -> None:
        ...
        if self._running:
            logger.warning(
                f"GameDataSyncService for agent {self.agent_id} is already running"
            )
            return
    ...
    async def stop(self) -> None:
        ...
        self.udp_dispatcher.unsubscribe(
            "entity_operation", self._handle_entity_operation
        )
    ...
    def _handle_entity_operation(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(
                f"🔔 UDP: entity_operation received - op={op}, key={entity_key}, name={entity_name}"
            )
            self._sync_queue.put_nowait(("entity_operation", payload))
            logger.info(
                f"   Queued for processing (queue size: {self._sync_queue.qsize()})"
            )
        except asyncio.QueueFull:
            logger.warning(
                f"Sync queue full, dropping entity operation: {payload.get('op')}"
            )
    ...
    def _handle_file_io(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Sync queue full, dropping file I/O: {payload.get('file_type')}"
            )
    ...
    async def _process_update(self, update_type: str, payload: Dict[str, Any]) -> None:
        ...
        try:
            logger.info(
                f"⚙️  Processing update: type={update_type}, op={payload.get('op', 'N/A')}"
            )
    ...
    async def _process_sync_queue(self, timeout: float = 5.0) -> None:
        ...
        if processed > 0:
            logger.debug(
                f"Processed {processed} queued updates for agent {self.agent_id}"
            )
    ...
    async def _load_chunk(self, chunk_x: int, chunk_y: int) -> None:
        ...
        try:
                ...
                with open(trees_rocks_file, "r") as f:
                    for line in f:
                        if line.strip():
                            ...
                            entity_key = (
                                data.get("key")
                                or f"{data['name']}:{data['position']['x']},{data['position']['y']}"
                            )
                    ...
                    for line in f:
                        if line.strip():
                            ...
                            if op == "upsert":
                                ...
                                await self._sync_entity_created(
                        ...
                        if line.strip():
                            ...
                            if op == "remove":
                                ...
                                if entity_key:
                                    ...
                                    logger.debug(
                                        f"Removed resource entity {entity_key} from chunk ({chunk_x}, {chunk_y})"
                                    )
            ...
            logger.error(
                f"Error loading chunk ({chunk_x}, {chunk_y}): {e}", exc_info=True
            )
            raise
    ...
    async def _sync_entity_created(self, payload: Dict[str, Any]) -> None:
        ...
        logger.info(
            f"🔧 _sync_entity_created called with payload keys: {list(payload.keys())}"
        )
        ...
        logger.info(
            f"🔧 Entity data found: {entity_data.get('name')} at {entity_data.get('position')}"
        )
        ...
        if not self._is_valid_entity(entity_name):
            logger.debug(
                f"Skipping entity {entity_name} (not in placeable_entity ENUM)"
            )
            return
    ...
    async def _sync_entity_destroyed(self, payload: Dict[str, Any]) -> None:
        ...
        logger.debug(
            f"🗑️  Processing entity_destroyed - key={entity_key}, name={entity_name}"
        )
        ...
        is_resource_entity = (
            "tree" in entity_name
            or "rock" in entity_name
            or entity_name.startswith("dead-")
        )
        ...
        if is_resource_entity:
            ...
            self.db.execute(
                "DELETE FROM transport_belt WHERE entity_key = ?", [entity_key]
            )
            self.db.execute(
                "DELETE FROM mining_drill WHERE entity_key = ?", [entity_key]
            )
            ...
            self.db.execute(
                "DELETE FROM electric_pole WHERE entity_key = ?", [entity_key]
            )
    ...
    async def _handle_file_written(
        ...
        if file_type == "resource":
            ...
            logger.debug(
                f"entities_init file written (handled via entity operations): {file_path}"
            )
    ...
    async def _handle_file_appended(
        ...
        if file_type == "power_statistics":
            ...
            await self._append_agent_production_statistics(
                file_path_obj, agent_id, entry_count
            )
    ...
    async def _append_agent_production_statistics(
            ...
            for line in lines[-entry_count:]:
                if line.strip():
                    try:
                        ...
                        logger.warning(
                            f"Error parsing agent production statistics entry: {e}"
                        )
                        continue
        ...
        try:
            ...
            if rows:
                ...
                logger.debug(
                    f"Appended {len(rows)} agent production statistics entries for agent {agent_id_int}"
                )
        except Exception as e:
            # Table might not exist
    ...
    async def _process_snapshot_state(self, payload: Dict[str, Any]) -> None:
        ...
        logger.debug(f"Chunk ({chunk_x}, {chunk_y}) state: {old_state} -> {state}")
    ...
    def get_sequence_gap_stats(self) -> Dict[str, Any]:
        ...
        if total_gaps > 0:
            ...
            for event_type, gaps in sorted(
                gaps_by_type.items(), key=lambda x: len(x[1]), reverse=True
            ):
                total_missed = sum(g["gap_size"] for g in gaps)
                summary_lines.append(
                    f"  {event_type}: {len(gaps)} gaps, {total_missed} packets missed"
                )
