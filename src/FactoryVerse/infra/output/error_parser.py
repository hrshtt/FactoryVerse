...
class FactorioErrorParser:
    ...
    def _parse_lua_error(self, error_text: str) -> ParsedError:
        ...
        for i, line in enumerate(lines):
            ...
            elif not error_message and (
                "error" in line_stripped.lower() or ":" in line_stripped
            ):
    ...
    def _filter_frames(self, frames: List[str]) -> List[str]:
        ...
        for frame in frames:
            is_internal = any(
                re.search(pattern, frame) for pattern in self.INTERNAL_FRAME_PATTERNS
            )
            if not is_internal:
