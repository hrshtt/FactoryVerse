...
class AsyncActionListener:
    ...
    def __init__(
        self, udp_dispatcher: Optional[UDPDispatcher] = None, timeout: int = 30
    ):
        """
        ...
        ] = {}  # Store event loop for each action
        self.running = False
    ...
    def _handle_udp_message(self, payload: Dict[str, Any]):
        ...
        try:
            ...
            elapsed_ticks = (
                (completion_tick - rcon_tick)
                if (rcon_tick and completion_tick)
                else None
            )
            ...
            print(
                f"✅ {payload.get('action_type', 'action')} completed{elapsed_str}: {action_id} [processed in {process_time_ms:.2f}ms]"
            )
    ...
    async def wait_for_action(
        self, action_id: str, timeout: Optional[int] = None
    ) -> Dict[str, Any]:
        """
...
class RconHelper:
    ...
    def refresh_interfaces(self) -> None:
        """Refresh the list of remote interfaces."""
        self._fetch_interfaces()
    ...
    def _fetch_interfaces(self):
        ...
        try:
            ...
            if not interfaces_json.strip():
                raise ValueError(
                    "RCON command returned empty string - check if helpers.table_to_json is available"
                )

            self.interfaces = json.loads(interfaces_json)
            # print(f"✅ Loaded remote interfaces: {list(self.interfaces.keys())}") # Reduce noise
    ...
    def get_cmd(self, category: str, method: str, args: Dict[str, Any]) -> str:
        ...
        assert method in self.interfaces[category], (
            f"Unknown method: {category}.{method}"
        )
        assert isinstance(args, dict), "Args must be a dict"

        # Use helpers.json_to_table to convert JSON string to Lua table
        remote_call = f"remote.call('{category}', '{method}', helpers.json_to_table('{json.dumps(args)}'))"
    ...
    def run(
        ...
        """
        cmd = (
            self.get_safe_cmd(category, method, args)
            if safe
            else self.get_cmd(category, method, args)
        )
    ...
    def pause(self):
        """Pause the game."""
        res = self.rcon_client.send_command(
            "/c rcon.print(game.tick_paused); game.tick_paused=true; rcon.print(game.tick_paused)"
        )
        print(res)
    ...
    def resume(self):
        """Resume the game."""
        res = self.rcon_client.send_command(
            "/c rcon.print(game.tick_paused); game.tick_paused=false; rcon.print(game.tick_paused)"
        )
        print(res)
