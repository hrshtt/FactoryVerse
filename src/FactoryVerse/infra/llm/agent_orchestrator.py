...
class FactorioAgentOrchestrator:
    ...
    async def run_turn(self, user_message: str) -> str:
        ...
        for iteration in range(1, 11):
            ...
            # Execute tool calls
            logger.info(
                f"Turn {self.turn_number}: Executing {len(response.tool_calls)} tool calls..."
            )
                ...
                try:
                    ...
                    if not validation.valid:
                        ...
                        result, status = await self._execute_tool(
                            tool_name, validation.parsed_arguments
                        )
    ...
    async def _check_notifications(self) -> list[Dict[str, Any]]:
        ...
        Executes code in the Jupyter kernel to drain notifications from the
        AgentRuntime's listener queue. Notifications are generated by the Lua
        mod (e.g., research_finished events) and received via UDP.
        ...
        """
        # Execute code in kernel to drain notifications from the listener
        # The `runtime` variable is available in the kernel namespace from boilerplate
        notification_code = """
import json
try:
    # Get notifications from the AsyncActionListener (non-blocking)
    notifs = await runtime._listener.get_notifications(timeout=0.05)
    # Return as JSON for parsing
    print(json.dumps(notifs))
except Exception as e:
    # Return empty list on error (listener may not be started)
    print("[]")
"""
        try:
    ...
    async def _add_notifications_to_messages(self):
        ...
        if notifications:
            logger.info(
                f"Found {len(notifications)} pending notification(s) after tool execution"
            )
    ...
    def _check_and_compress_context(self):
        ...
        if len(self.messages) > 1:
            ...
            if (
                get_role(second_msg) == "user"
                and "initial game state" in get_content(second_msg).lower()
            ):
