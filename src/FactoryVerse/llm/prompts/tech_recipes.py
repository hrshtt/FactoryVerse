...
class TechRecipePromptGenerator:
    ...
    def get_available_technologies(self, researched: List[str]) -> List[TechInfo]:
        ...
        for tech_name, tech_data in self.data["technology"].items():
            ...
            unlocks = [
                e["recipe"]
                for e in tech_data.get("effects", [])
    ...
    def get_enabled_recipes(self, enabled_recipe_names: List[str]) -> List[RecipeInfo]:
        ...
        for recipe_name in enabled_recipe_names:
            ...
            if "results" in recipe_data:
                ...
                products.append(
                    (recipe_data["result"], recipe_data.get("result_count", 1))
                )
    ...
    def generate_tech_section(self, researched: List[str], limit: int = 10) -> str:
        ...
        for tech in available[:limit]:
            ...
            if tech.has_trigger:
                lines.append(
                    f"- **Unlocked by**: {tech.trigger_desc} (automatic, ie doesnt need to be added to research queue)"
                )
            else:
                lines.append(f"- **Research**: {tech.cycles} cycles")
                if tech.science_packs:
                    packs_str = ", ".join(
                        [f"{amt}x {name}" for name, amt in tech.science_packs]
                    )
                    lines.append(f"- **Packs/cycle**: {packs_str}")
                    total_str = ", ".join(
                        [
                            f"{amt * tech.cycles}x {name}"
                            for name, amt in tech.science_packs
                        ]
                    )
    ...
    def generate_recipe_section(
        self, enabled_recipe_names: List[str], limit: int = 15
    ) -> str:
        ...
        if handcraftable:
            ...
            for recipe in handcraftable:
                ing_str = " + ".join(
                    [f"{amt}x {name}" for name, amt in recipe.ingredients]
                )
                prod_str = " + ".join(
                    [f"{amt}x {name}" for name, amt in recipe.products]
                )
                lines.append(
                    f"- **{recipe.name}** [🖐️ handcraftable]: {ing_str} → {prod_str} ({recipe.time}s)"
                )
            lines.append("\n")
        ...
        if smelting:
            ...
            lines.append(
                "*Note: Smelting requires a furnace (stone-furnace, steel-furnace, or electric-furnace)*\n\n"
            )
            for recipe in smelting:
                ...
                lines.append(
                    f"- **{recipe.name}** [🔥 smelting - requires furnace]: {ing_str} → {prod_str} ({recipe.time}s)"
                )
            lines.append("\n")
        ...
        if other_machine:
            ...
            for recipe in other_machine:
                ...
                lines.append(
                    f"- **{recipe.name}** [⚙️ {recipe.category}]: {ing_str} → {prod_str} ({recipe.time}s)"
                )
            lines.append("\n")
