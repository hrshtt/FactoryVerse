...
class ToolValidator:
    ...
    DANGEROUS_IMPORTS = {
        ...
        "__import__",
        "compile",
        "open",  # open is allowed in DSL context but not in user code
        "execfile",
        "input",
        "raw_input",
    }
    ...
    def __init__(self, max_code_length: int = 10000, max_query_length: int = 5000):
        """
    ...
    def validate_tool_call(
        ...
        if tool_name == "execute_dsl":
            ...
            return ValidationResult(
                valid=True, parsed_arguments=arguments, error=None, warnings=[]
            )
    ...
    def validate_dsl_code(self, code: str) -> ValidationResult:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                if node.module and any(
                    danger in node.module for danger in self.DANGEROUS_IMPORTS
                ):
                    return ValidationResult(
                        valid=False,
                        parsed_arguments=None,
        ...
        return ValidationResult(
            valid=True, parsed_arguments={"code": code}, error=None, warnings=warnings
        )
    ...
    def validate_duckdb_query(self, query: str) -> ValidationResult:
        ...
        if query_upper.count("JOIN") > 5:
            warnings.append(
                f"Many JOINs detected ({query_upper.count('JOIN')}) - may be slow"
            )

        return ValidationResult(
            valid=True, parsed_arguments={"query": query}, error=None, warnings=warnings
        )
