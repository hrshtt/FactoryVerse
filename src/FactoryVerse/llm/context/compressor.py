...
class OutputCompressor:
    ...
    def __init__(self, default_max_chars: int = 5000, default_max_rows: int = 100):
        """
    ...
    def compress_query_result(
        ...
        if separator_idx is not None and separator_idx > 0:
            ...
            if len(data_lines) > max_rows:
                ...
                # Rebuild output
                compressed_lines = (
                    header_lines + [lines[separator_idx]] + truncated_data
                )
                compressed_lines.append(f"\n[{omitted_count} more rows omitted]")

                if total_rows:
                    compressed_lines.insert(
                        0, f"Showing {max_rows} of {total_rows} rows"
                    )
    ...
    def compress_action_result(
        ...
        for line in lines:
            if any(
                re.match(pattern, line, re.IGNORECASE) for pattern in preserve_patterns
            ):
    ...
    def compress_error(self, error: str, max_chars: int = 1000) -> CompressedOutput:
        """
