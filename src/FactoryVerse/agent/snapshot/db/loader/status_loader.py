...
@lru_cache(maxsize=1)
def _get_entity_name_enum() -> Dict[str, int]:
    """
    Get entity name to enum mapping dynamically from PrototypeDataManager.

    This MUST match the order used by the Lua side when set_entity_filter() is called.
    The entity list is sorted alphabetically by the prototype manager.
    """
    ...
    # Build enum mapping: entity_name -> index (0-indexed)
    return {entity_name: idx for idx, entity_name in enumerate(entity_list)}


def _get_entity_enum_to_name() -> Dict[int, str]:
    """Reverse mapping: enum value -> entity name."""
    return {v: k for k, v in _get_entity_name_enum().items()}


# DEPRECATED: Legacy hardcoded enum for reference/fallback only
# This is no longer used - we dynamically load from PrototypeDataManager
_LEGACY_ENTITY_NAME_ENUM = {
    "accumulator": 0,
...
# Dynamic mappings - call functions to get current values
# These replace the old hardcoded ENTITY_NAME_ENUM and ENTITY_ENUM_TO_NAME
...
def _decode_status_record(
    ...
    Returns:
        (entity_name, status_name, x, y) or None if invalid
    """
    if len(record) != 4:
        return None
    ...
    return (entity_name, status_name, x, y)
...
def load_status_file(
    ...
    status_records = []
    entity_enum_to_name = _get_entity_enum_to_name()
        ...
        for line in f:
            if line.strip():
                try:
                    ...
                    if decoded:
                        entity_name, status_name, x, y = decoded
    ...
    # Uses composite key (entity_name, x, y, tick) for uniqueness
    ...
    con.execute("""
        CREATE TEMPORARY TABLE temp_entity_status (
            ...
            x DOUBLE NOT NULL,
            y DOUBLE NOT NULL,
            PRIMARY KEY (entity_name, x, y, tick)
        );
    """)
    ...
    for record in status_records:
        con.execute(
            """
            INSERT INTO temp_entity_status (entity_name, tick, status, x, y)
            VALUES (?, ?, ?::status, ?, ?)
            """,
            [
                record["entity_name"],
...
def create_status_view(con: duckdb.DuckDBPyConnection) -> None:
    ...
    Note: Joins on composite key (entity_name, position_x, position_y) since
    map_entity doesn't have entity_key column (uses composite PK instead).
    """
    # Create view that joins with map_entity for full context
    # Join on composite key (entity_name, position_x, position_y)
    # Use small tolerance for floating point position matching
    con.execute("""
        ...
        SELECT 
            ...
            es.x as position_x,
            es.y as position_y,
            me.entity_name as entity_name_full,
            me.position_x as entity_position_x,
            me.position_y as entity_position_y,
            me.bbox_min_x,
            me.bbox_min_y,
            me.bbox_max_x,
            me.bbox_max_y,
        ...
        LEFT JOIN map_entity me 
            ON me.entity_name = es.entity_name
            AND ABS(me.position_x - es.x) < 0.01
            AND ABS(me.position_y - es.y) < 0.01;
    """)
...
class StatusSubscriber:
    ...
    def get_updates(self) -> List[Dict]:
        ...
        if self.entity_filters:
            # Filter by entity_name directly (no need to parse entity_key)
            entity_list = list(self.entity_filters)
            conditions.append(f"entity_name IN ({', '.join(['?'] * len(entity_list))})")
        ...
        query = f"""
            SELECT entity_name, tick, status, x, y
