...
def load_all(
    ...
    *,
    ...
    Args:
        ...
        replay_updates: If True, replay operations logs (entities-updates.jsonl, ghosts-updates.jsonl)
    """
    ...
    if include_derived:
        ...
        print("=" * 60)
        load_derived_tables(con, snapshot_dir)
...
def load_all_to_file(
    ...
    **kwargs,
    ...
    con = connect(db_path)
    load_all(con, snapshot_dir, prototype_api_file, **kwargs)
    return con
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python -m src.FactoryVerse.infra.db.loader.main <snapshot_dir> [db_path]"
        )
        sys.exit(1)
    ...
    load_all_to_file(snapshot_dir, db_path)
