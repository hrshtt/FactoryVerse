...
--- Validate basic cancel parameters
--- @param params table
--- @return boolean, string|nil
local function validate_cancel_params(params)
    if type(params) ~= "table" then 
        return false, "params must be a table" 
    end
    if type(params.agent_id) ~= "number" then 
...
--- @param params table
--- @return boolean, string|nil
local function validate_cancel_recipe_exists(params)
    if not params.recipe then return true end  -- Let other validators handle missing recipe
...
local cancel_validators = { 
    ...
    validate_cancel_recipe_exists, 
    validate_has_crafting, 
    validate_recipe_matches_tracking 
}

return {
    enqueue = enqueue_validators,
    cancel = cancel_validators
}
